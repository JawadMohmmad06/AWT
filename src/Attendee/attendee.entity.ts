...
export class AttendeeEntity{
    ...
    Username:string;
    @IsNotEmpty()
    @Matches(/^(?=.*[a-z])(?=.*[A-Z])/,{message:'The password must contain at least one uppercase letter and one lowercase letter'})
    @Column()
    Password:string;
    @IsEmail()
    @IsNotEmpty()
    ...
    PhotoName:string;
    @Column()
    @Matches(/^(01\d{9})$/, { message: 'The phone number must start with "01" and contain 11 digits without any other characters.' })
    @IsNotEmpty()
    @Column()
    Phonenumber:string
    @OneToMany(()=>BookedEventEntity,bookedEventEntity=>bookedEventEntity.Attendee,{cascade:true})
...
export class UpdateAttendeeEntity{
